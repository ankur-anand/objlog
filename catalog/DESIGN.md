# objlog/catalog Design

`objlog/catalog` is the partition-level metadata system for committed
immutable segments.
...
Shared metadata types live in `objlog/pmeta`. `catalog` owns the durable
...
`objlog/catalog` is the common catalog API. It contains errors, request
...
- `objlog/catalog/blob`: object-store catalog using immutable JSON pages
  and a CAS-protected head object.
- `objlog/catalog/blob/s3`, `objlog/catalog/blob/gcs`,
  `objlog/catalog/blob/azure`: provider backends for the blob catalog.
- `objlog/catalog/cache/redis`: optional read-through Redis cache
  decorator for catalog readers.
- `objlog/catalog/notify`: optional commit hint emitters for writer
  sessions.
- `objlog/catalog/watch`: optional hint consumers/cache warmers.
- `objlog/catalog/writeradapter`: adapter from catalog writer sessions to
  the public partition writer session interface.

Future Cassandra/Scylla support should live under `objlog/catalog/cql`.
