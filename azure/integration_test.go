...
func newAzuriteStore(t testing.TB, ctx context.Context) *plazure.Store {
	t.Helper()
	containerName := getenv("OBJLOG_AZURITE_CONTAINER", "objlog-lifecycle-it")
	connectionString := getenv("OBJLOG_AZURITE_CONNECTION_STRING", getenv("CATALOG_BLOB_AZURITE_CONNECTION_STRING", defaultAzuriteConnectionString))
