# blob/sink Integration Tests

Live integration tests are opt-in. Normal `go test ./blob/sink/...`
...
```sh
go test ./blob/sink/s3 ./blob/sink/azure \
...
- abort behavior;
- `sink.Factory -> segwriter` end-to-end segment upload;
...
- `BLOB_SINK_MINIO_ENDPOINT=http://127.0.0.1:9000`
- `BLOB_SINK_MINIO_BUCKET=blob-sink-it`
- `BLOB_SINK_MINIO_ACCESS_KEY` falls back to `MINIO_ROOT_USER`, then `minioadmin`
- `BLOB_SINK_MINIO_SECRET_KEY` falls back to `MINIO_ROOT_PASSWORD`, then `minioadmin`

Azurite defaults:

- `BLOB_SINK_AZURITE_CONTAINER=blob-sink-it`
- `BLOB_SINK_AZURITE_CONNECTION_STRING` defaults to the Azurite
