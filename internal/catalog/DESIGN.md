# objlog/internal/catalog Design

`objlog/internal/catalog` is the partition-level metadata system for committed
immutable segments.
...
Shared metadata types live in `objlog/internal/pmeta`. `catalog` owns the durable
...
- `objlog/internal/catalog/blob`: object-store catalog using immutable JSON pages
  and a CAS-protected head object.
- `objlog/internal/catalog/blob/s3`, `objlog/internal/catalog/blob/gcs`,
  `objlog/internal/catalog/blob/azure`: provider backends for the blob catalog.
- `objlog/internal/catalog/cache/redis`: optional read-through Redis cache
  decorator for catalog readers.
- `objlog/internal/catalog/notify`: optional commit hint emitters for writer
  sessions.
- `objlog/internal/catalog/watch`: optional hint consumers/cache warmers.
- `objlog/internal/catalog/writeradapter`: adapter from catalog writer sessions to
  the public partition writer session interface.

Future Cassandra/Scylla support should live under `objlog/internal/catalog/cql`.
...
The naming contract is defined in `objlog/OBJECT_LAYOUT.md`. Page refs
