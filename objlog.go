...
// Log is one objlog client over one configured store.
type Log struct {
	mu      sync.Mutex
	store   Store
	metrics Metrics
...
func Open(opts Options) (*Log, error) {
	if opts.Store == nil {
		return nil, fmt.Errorf("objlog: nil store")
	}
...
func (l *Log) checkOpen() error {
	if l == nil {
		return fmt.Errorf("objlog: nil log")
	}
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.closed {
...
func (l *Log) NewReader(opts ReaderOptions) (*Reader, error) {
	if l == nil {
		return nil, fmt.Errorf("objlog: nil log")
	}
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.closed {
...
func (l *Log) InitializePartition(ctx context.Context, opts InitializePartition) (InitializePartitionResult, error) {
	...
	if manager == nil {
		return InitializePartitionResult{}, fmt.Errorf("objlog: nil writer catalog")
	}
...
func (l *Log) RequestRetention(ctx context.Context, request RetentionRequest) (RetentionRequestState, error) {
	...
	if manager == nil {
		return RetentionRequestState{}, fmt.Errorf("objlog: nil retention catalog")
	}
...
func (l *Log) OpenWriter(ctx context.Context, opts WriterOptions) (*Writer, error) {
	...
	if catalogWriterManager == nil {
		return nil, fmt.Errorf("objlog: nil writer catalog")
	}
	sinkFactory := l.store.SinkFactory()
	if sinkFactory == nil {
		return nil, fmt.Errorf("objlog: nil sink factory")
	}
...
func newReader(store Store, opts ReaderOptions, metrics Metrics) (*Reader, error) {
	...
	if cat == nil {
		return nil, fmt.Errorf("objlog: nil reader catalog")
	}
...
func validateWriterOptions(opts WriterOptions) error {
	...
	case opts.Batch.MaxDelay < 0:
		return fmt.Errorf("objlog: negative batch max delay %s", opts.Batch.MaxDelay)
	case opts.Backpressure.MaxPendingBatches < 0:
		return fmt.Errorf("objlog: negative max pending batches %d", opts.Backpressure.MaxPendingBatches)
	case opts.Timeouts.SegmentFinalize < 0:
		return fmt.Errorf("objlog: negative segment finalize timeout %s", opts.Timeouts.SegmentFinalize)
	case opts.Timeouts.CatalogPublish < 0:
		return fmt.Errorf("objlog: negative catalog publish timeout %s", opts.Timeouts.CatalogPublish)
	default:
...
func validateWriterPipelineOptions(opts WriterPipelineOptions) error {
	...
	case opts.BlockBytes < 0:
		return fmt.Errorf("objlog: negative block bytes %d", opts.BlockBytes)
	case opts.PartBytes < 0:
		return fmt.Errorf("objlog: negative part bytes %d", opts.PartBytes)
	case opts.SealParallelism < 0:
		return fmt.Errorf("objlog: negative seal parallelism %d", opts.SealParallelism)
	case opts.BlockBuffers < 0:
		return fmt.Errorf("objlog: negative block buffers %d", opts.BlockBuffers)
	case opts.UploadParallelism < 0:
		return fmt.Errorf("objlog: negative upload parallelism %d", opts.UploadParallelism)
	case opts.UploadQueueSize < 0:
		return fmt.Errorf("objlog: negative upload queue size %d", opts.UploadQueueSize)
	default:
		return nil
	}
}
